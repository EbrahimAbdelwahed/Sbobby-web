...
function QuestionPanel({
  ...
  answered: boolean;
  onToggle: () => void;
  onRate: (rating: Rating, selectedOptionId?: string | null) => void;
}) {
      ...
      {showAnswer && question.explanation ? (
        <div className="mt-6 border-t border-[var(--sb-border)] pt-6">
          <div className="sb-answer-box">
            ...
            {selectedOptionId ? (
              <p className={`mt-3 text-sm font-semibold ${selectedIsCorrect ? "text-emerald-700" : selectedIsCorrect === false ? "text-rose-700" : "text-[var(--sb-text-dim)]"}`}>
                {selectedIsCorrect == null ? "Risposta non correggibile automaticamente: valuta manualmente." : selectedIsCorrect ? "Corretto." : "Sbagliato."}
              </p>
            ) : null}
